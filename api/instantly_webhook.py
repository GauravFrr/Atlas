...
from fastapi import FastAPI, Header, HTTPException, Request  # noqa: F401 — Request reused
...
async def instantly_webhook(
    ...
    if event and event not in ("reply_received", "auto_reply_received"):
    ...
    if lead_id and label in ("interested", "unknown", "not_now"):
        ...
        await notifier.send_hot_reply(email or "lead", subject, body[:200])
    ...
    if eid:
        ...
        if STATE_FILE.is_file():
            try:
                seen = set(json.loads(STATE_FILE.read_text()).get("seen_email_ids") or [])
            except Exception:
                pass
        seen.add(eid)
    ...
    logger.success(f"[Webhook] {email} → {label}")
    return {"status": "ok", "classification": label}
