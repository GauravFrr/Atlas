...
def _run_webhook_fastapi(app: Application, settings: Settings, webhook_url: str) -> None:
    import uvicorn
    ...
    fastapi_app = FastAPI(title="Atlas Telegram", lifespan=lifespan)

    @fastapi_app.get("/health")
    async def health() -> dict[str, str]:
        return {"status": "ok", "service": "telegram"}

    @fastapi_app.post(hook_path)
...
def run_bot(app: Application, settings: Settings) -> None:
    ...
    if settings.telegram_webhook_enabled:
        ...
        if not webhook_url:
            ...
            _run_polling(app)
            return
        _run_webhook_fastapi(app, settings, webhook_url)
        return
